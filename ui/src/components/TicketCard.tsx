...
export default function TicketCard({ ticket, onDeleteRequest, onEditRequest }: TicketCardProps) {
  // Telemetry processing timer calculation
  ...
  return (
    <div className={`bg-slate-800 border rounded-xl p-5 shadow-sm transition-all hover:border-slate-600 ${
      isSystemFault ? 'border-rose-500/40 shadow-rose-950/10' : 'border-slate-700/80'
    }`}>
      
      {/* 🚨 NEW: CRITICAL INFRASTRUCTURE WARNING BANNER */}
      {isSystemFault && (
      ...
      {/* Autonomous AI Agent Metadata Container */}
      {(ticket.assignedLabel || ticket.agentReply) && (
        <div className={`border-t pt-3 mt-3 rounded-lg p-3 border space-y-2 ${
          isSystemFault ? 'bg-rose-950/10 border-rose-900/30' : 'bg-indigo-950/20 border-indigo-900/30'
        }`}>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <span className={`text-xs px-2 py-0.5 rounded font-bold uppercase ${
                isSystemFault ? 'bg-rose-500/20 text-rose-300' :
            ...
            {ticket.status === 'Resolved' && (
              <span className="bg-indigo-500/20 text-indigo-300 text-xs px-2 py-0.5 rounded font-mono font-bold flex items-center gap-1">
                ⏱️ Latency: {calculateLatency(ticket.createdAt, ticket.resolvedAt)}
              </span>
            )}
          </div>
      ...
      {(ticket.status === 'Resolved' || ticket.status === 'Failed') && (
        <div className="flex justify-end items-center gap-3 border-t border-slate-700/30 pt-3 mt-4">
          ...
          {/* Hide edit option for system crashes, only allow engineers to clear the record */}
          {!isSystemFault && (
            <button
          ...
          >
            <svg xmlns="http://w3.org" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-rose-500 flex-shrink-0">
              <path fillRule="evenodd" d="M16.5 4.478v.227a48.816 48.816 0 0 1 3.878.512.75.75 0 1 1-.256 1.478l-.209-.035-1.005 13.07a3 3 0 0 1-2.991 2.77H8.084a3 3 0 0 1-2.77L4.087 6.66l-.209.035a.75.75 0 0 1-.256-1.478A48.567 48.567 0 0 1 7.5 4.705v-.227c0-1.564 1.213-2.9 2.816-2.951a52.662 52.662 0 0 1 3.369 0c1.603.051 2.815 1.387 2.815 2.951Zm-6.136-1.452a51.196 51.196 0 0 1 3.273 0C14.39 3.05 15 3.684 15 4.478v.113a49.488 49.488 0 0 0-6 0v-.113c0-.794.609-1.428 1.364-1.452Zm-.355 5.945a.75.75 0 1 0-1.5.058l.347 9a.75.75 0 1 0 1.499-.058l-.346-9Zm5.48.058a.75.75 0 1 0-1.498-.058l-.347 9a.75.75 0 0 0 1.5.058l.345-9Z" clipRule="evenodd" />
            </svg>
            <span>{isSystemFault ? 'Acknowledge & Clear' : 'Delete Record'}</span>
          </button>
